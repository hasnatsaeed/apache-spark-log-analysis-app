 ...
 */
public class ApacheAccessLogAnalyzerUsingSparkCore {
  ...
  public static void main(String[] args) {
    ...
    SparkConf conf = new SparkConf().setMaster("local[2]").setAppName("Log Analyzer");
   ...
   /* if (args.length == 0) {
    ...
    JavaRDD<LogLineModel> accessLogs =
       logLines.map(LogLineModel::parseFromLogLine).cache();

    // Calculate statistics based on the content size.
    // Note how the contentSizes are cached as well since multiple actions
    //   are called on that RDD.
   /* JavaRDD<String> contentSizes =
...
*/
    ...
    List<Tuple2<String, Long>> responseCodeToCount =
        accessLogs.mapToPair(log -> new Tuple2<>(log.getHttpResponseCode(), 1L))
...
*/
    ...
    List<Tuple2<String, Long>> topEndpoints = accessLogs
        .mapToPair(log -> new Tuple2<>(log.getServerEndpoint(), 1L))
        .reduceByKey(SUM_REDUCER)
